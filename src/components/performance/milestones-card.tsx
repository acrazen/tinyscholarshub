  ...
  return (
    <Card className="shadow-md rounded-xl">
      <CardHeader>
        ...
        <CardDescription className="text-sm">Tracking progress in key areas.</CardDescription>
      </CardHeader>
      <CardContent>
        {milestones.length > 0 ? (
          <ScrollArea className="h-[22rem] sm:h-72"> {/* Adjusted height for mobile consistency */}
                  ...
                  <div className="flex-grow">
                    <div className="flex justify-between items-start mb-0.5">
                      <h4 className="font-semibold text-sm">{milestone.title}</h4>
                       <Badge variant="outline" className={`text-xs px-1.5 py-0.5 sm:px-2 ${categoryColors[milestone.category]}`}>{milestone.category}</Badge>
                    </div>
                    <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">{milestone.description}</p>
